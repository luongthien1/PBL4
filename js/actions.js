$(document).ready(function(){
	...
	*/
	$("body").delegate(".category","click",function(event){
		...
		var amount = $(".input-select-number").find(":selected").text();
		var sort = $(".input-select-sort").find(":selected").text();
			$.ajax({
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_seleted_Category:1,cat_id:cid,get_amount:amount,get_sort:sort},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("body").delegate(".selectBrand","click",function(event){
		...
		var sort = $(".input-select-sort").find(":selected").text();
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{selectBrand:1,brand_id:bid,get_amount:amount,get_sort:sort},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("#search_btn").click(function(){
		...
		if(keyword != ""){
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{search:1,keyword:keyword,get_amount:amount,get_sort:sort},
			success	:	function(data){ 
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
